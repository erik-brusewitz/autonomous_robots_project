...
const int PORPHOLOGY_EX_ITERATIONS = 2;
...
std::tuple<std::vector<std::vector<cv::Point>>, std::vector<cv::Point>> detectConeContours(const cv::Mat &img, 
    ...
    for (int counter=0; counter<morphologyExIter; counter++) 
    {
        cv::morphologyEx(HSVImgFiltered, HSVImgFiltered, cv::MORPH_OPEN, element, cv::Point(-1,-1), 1, 1, 1);
        cv::morphologyEx(HSVImgFiltered, HSVImgFiltered, cv::MORPH_CLOSE, element, cv::Point(-1,-1), 3, 1, 1);
        cv::morphologyEx(HSVImgFiltered, HSVImgFiltered, cv::MORPH_DILATE, element, cv::Point(-1, -1), 2, 1, 1);
    }
    cv::Mat maskedImg(img.size().height, img.size().width, CV_8UC4, cv::Scalar(0,0,0));
    HSVImgFiltered.copyTo(maskedImg, mask);
...
std::string convertToNormalizedCsvString(std::vector<cv::Point> coneCenters, unsigned int imgWidth, unsigned int imgHeight) {
    ...
    for (auto coneCenter : coneCenters) {
        csvString.append(std::to_string(((float)coneCenter.y)/((float)(imgHeight))) + "," + std::to_string(((float)coneCenter.x)/((float)(imgWidth))) + ";");
    }
    return csvString;
}
                ...
                int prevHorizon)
{
    ...
    cv::Moments m = moments(HSVImgFiltered,true);
    
    return std::max(maxHorizon, (int)((1-MOVING_AVERAGE_COEFF_HORIZON)*prevHorizon + MOVING_AVERAGE_COEFF_HORIZON*(m.m01/m.m00)));
}
...
int32_t main(int32_t argc, char **argv) {
    ...
    if ( (0 == commandlineArguments.count("cid")) ||
        ...
        int morphologyExIter = commandlineArguments.count("morphologyExIter") ? std::stoi(commandlineArguments["morphologyExIter"]) : PORPHOLOGY_EX_ITERATIONS;
        ...
        if (sharedMemory && sharedMemory->valid()) {
            ...
            if (VERBOSE) 
            {
                ...
                if (TUNE)
                {
                    ...
                    cv::createTrackbar("morphologyExIter", TUNNING_WINDOW_NAME, &morphologyExIter, 10);
            ...
            while (od4.isRunning()) {
                ...
                if (adaptiveHorizon) {horizon = findHorizon(img,
                ...
                std::tie(blueConeContours, blueConeCenters) = detectConeContours(img, blueConesDetectionMask,
                                                    cv::Scalar(hLowBlue, sLowBlue, vLowBlue), 
                                                    cv::Scalar(hHighBlue, sHighBlue, vHighBlue),
                                                    morphologyExIter);

                std::tie(yellowConeContours, yellowConeCenters) = detectConeContours(img, yellowConesDetectionMask, 
                                                    cv::Scalar(hLowYellow, sLowYellow, vLowYellow), 
                                                    cv::Scalar(hHighYellow, sHighYellow, vHighYellow),
                                                    morphologyExIter);
                ...
                if (VERBOSE) {
                    std::cout<<"Blue:  "<<blueConeCentersString<<"\nYellow:  "<<yellowConeCentersString<<"\nRed:  "<<redConeCentersString<<std::endl<<std::endl;
                    cv::Mat conesImg;
                    img.copyTo(conesImg);
